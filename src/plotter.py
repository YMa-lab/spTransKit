...
def plot_runtime(transformations : list):
    x = []
    y = []
...
def plot_memory(transformations : list):
    x = []
    y = []
    for i in range(1, len(transformations)):
        x.append(transformations[i].name)
...
def plot_gene_mean_var(transformations : list):
    ...
    plt.savefig("gene_mean_var.png")

def plot_marker_genes(transformations : list, genes : dict):
    for gene in genes.keys():
        ...
        plt.savefig(gene + "_distributions.png")

def plot_hvg_similarity(transformations : list, name : list):
    ...
    plt.savefig("hvg_similarity.png")

def plot_pathway_enrichment(enrichment : pandas.DataFrame):
    plt.figure(figsize = (16, 7.5))
...
def plot_sf_correspondence(t_one : list, t_two : list):
    fig = plt.figure(figsize = (16, 7.5))
    ...
    plt.savefig("sf_correspondence.png")

def plot_count_correspondence(t_one : list, t_two : list, genes : dict):
    for gene in genes.keys():
        fig = plt.figure(figsize = (16, 7.5))
        for t in range(0, len(t_one)):
            ax = fig.add_subplot(4, 5, t + 1)
        ...
        plt.savefig(gene + "_count_correspondence.png")

def plot_corr_matrix(transformations : list):
    fig = plt.figure(figsize = (16, 7.5))
    for t in range(0, len(transformations)):
        ax = fig.add_subplot(4, 5, t + 1)
...
def plot_total_rank(t_one : list, t_two : list, name : list, genes : dict, weight_matrix : pandas.DataFrame, enrichment : pandas.DataFrame) -> pandas.DataFrame:
